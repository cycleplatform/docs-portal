...
The first thing you'll want to understand, once you have a hub set up, is how to add a provider and a server.  A provider is (generally) a cloud IaaS provider, and a server is a virtual (or bare metal) machine.  You can add a provider by clicking on on `Hubs` and then `Integrations` in the left hand navigation, and then clicking the provider you'd like to and and [`Add Provider`](/reference/hubs/integrations/providers/) button.  Once you've added a provider, you can add a server by clicking on the `Servers` tab in the left hand navigation, and then clicking the [`Add Server`](/reference/infrastructure/servers#add-server) button.
